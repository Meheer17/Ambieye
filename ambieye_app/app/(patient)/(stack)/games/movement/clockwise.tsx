...
export default function ClockwiseGame() {
  ...
  // Prevent endGame running twice (timer fires + unmount)
  const isEndingRef = useRef(false);

  const { frameProcessor, result: eyeResult, reset: resetEyeTracking } = useEyeTracking(gameActive);

  // ── Cleanup helper — stops timer, animation, eye tracking ────────────────
  ...
  // ── Unmount: always clean up regardless of how the screen is left ─────────
  useEffect(() => {
    return () => {
      stopEverything();
    };
  ...
  // ── Back button: stop everything then navigate ────────────────────────────
      ...
      {!gameActive ? (
        <View style={styles.startContainer}>
          <Text style={styles.instructionText}>
            ...
            for 1 minute. This exercise helps strengthen your eye muscles and
