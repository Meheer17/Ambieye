...
describe("Picture Association Game Persistence Pipeline", () => {
  const mockSessionId = "sess_pa_test_12345";
  ...
  describe("1. GameSessionService Lifecycle", () => {
    it("starts a picture_association session with metadata and records game_started event", async () => {
      ...
      expect(session.gameId).toBe("picture_association");
    ...
    it("records granular round events: round_started, hint_used, answer_submitted, answer_correct, answer_incorrect, round_completed", async () => {
      (apiClient.post as jest.Mock).mockResolvedValue({
        data: { success: true, event_id: "evt_pa_1" },
      });
      ...
      await gameSessionService.recordEvent({
        ...
        metadata: {
          round: 1,
          difficulty: "easy",
          sourceItem: "Red Apple",
          choicesShown: ["Dinner Plate", "Car", "Phone", "Football"],
        },
      });
  ...
  describe("2. Companion Context Persistence Bridge", () => {
    it("ingests real game_completed event with telemetry into durable companion storage", async () => {
      ...
      expect(rawEvent.gameId).toBe("picture_association");
      ...
      expect(persistedEvents[0].payload.gameName).toBe("Picture Association");
      ...
      expect(context.recentGames[0].gameId).toBe("picture_association");
  ...
  describe("3. Caregiver Dashboard Integration", () => {
    it("persists CognitiveGameSession and retrieves via caregiverStorage", async () => {
      const caregiverSession: CognitiveGameSession = {
        ...
        completed: true,
        humanSummary: "Completed all 5 rounds linking natural everyday object associations with 92% accuracy.",
      };
      ...
      expect(saved?.gameName).toBe("Picture Association");
  ...
  describe("4. Offline & Non-Blocking Resilience", () => {
    it("handles backend API network errors gracefully without crashing or throwing", async () => {
      ...
      const session = await gameSessionService.startSession({
        sessionId: "sess_offline_pa_1",
        gameId: "picture_association",
      });
      expect(session.sessionId).toBe("sess_offline_pa_1");
      ...
      expect(event.sessionId).toBe("sess_offline_pa_1");
      ...
      await caregiverStorage.recordGameSession({
        id: "sess_offline_pa_cg",
        gameName: "Picture Association",
        iconEmoji: "🔗",
        timestamp: "Today · 05:50 PM",
        durationMinutes: 1,
        score: 450,
        accuracyPercent: 90,
        mistakes: 1,
      ...
      expect(sessions.some((s) => s.id === "sess_offline_pa_cg")).toBe(true);
    });
  });
});
